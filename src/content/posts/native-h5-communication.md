...
#### 2. JS Bridge 注入JS对象 (JavaScript Injection / JS Bridge)
1. **注入 (Injection):**
   - 当原生App初始化`WebView`并加载H5页面时，原生代码会直接向`WebView`的JavaScript环境中“注入”一个 **全局的JS对象** 。比如，注入一个名为`myAppBridge`的对象到`window`下。
   - 原生app中，会创建一个“联络员”对象`myAppBridge`，里面定义了可以给h5调用的各种方法
   - 在创建`WebView`并加载h5页面之前，原生APP会调用一个系统函数如`myWebView.addJavascriptInterface(new WebAppInterface(), "myAppBridge");`将`myAppBridge`注入到即将创建的h5中
   - h5一创建好就有联络员`myAppBridge`了，可以用这个对象直接调用里面的各种方法了，其实就是调用原生APP的方法了。
2. **H5调用 (H5 Call):** H5页面加载后，就可以像调用任何其他本地JS对象一样，直接调用这个被注入的对象上的方法。例如：
...
3. **原生处理 (Native Handling):** 这个`myAppBridge`对象上的每个方法都与一段原生代码绑定。当H5调用`takePhoto`时，实际上是触发了与之绑定的原生代码去执行。
4. **双向通信与回调 (Bidirectional & Callback):** 原生代码执行完任务（比如拍完照），可以再调用H5传递过来的回调函数，将结果（比如图片路径）作为参数传回去。这就轻松地实现了双向通信.
...
| 特性 | URL Scheme 拦截 | 注入JS对象 (JS Bridge) |
| --- | --- | --- |
| 核心思想 | 伪装成网络请求，由原生拦截解析 | 将原生能力封装成JS对象，供H5直接调用 |
| 通信方向 | 主要是单向 (H5 → 原生) | 双向 (H5 ↔ 原生) |
| 数据传递 | 简单数据，有长度限制 | 复杂对象、回调函数，无限制 |
| 性能 | 较差 | 优秀 |
| 主要用途 | 从外部浏览器唤起App，或作为非常基础的内部通信 | App内部H5与原生之间的主要通信方式 |
...
原文存档：[Notion 原文](<https://almond-oxygen-ceb.notion.site/h5-28d8b1cc19aa8046b9c9d2288677c43e?pvs=25>)
